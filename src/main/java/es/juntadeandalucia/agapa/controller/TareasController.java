...
import es.juntadeandalucia.agapa.dto.TareaMapper;
...
public class TareasController {
    ...
    @APIResponse(responseCode = "200", description = "Lista de tareas recuperada exitosamente", content = @Content(mediaType = MediaType.APPLICATION_JSON, schema = @Schema(implementation = TareaDTO.class, type = SchemaType.ARRAY)))
    public List<TareaDTO> listar() {
        return tareaMapper.toDTOList(tareasManager.listarTareas());
    }
    ...
    @APIResponses({
        @APIResponse(responseCode = "200", description = "Tarea marcada como completada", content = @Content(mediaType = MediaType.APPLICATION_JSON, schema = @Schema(implementation = TareaDTO.class))),
    ...
    public Response marcarDone(
        ...
        Tarea tarea = tareasManager.marcarComoDone(id);
        if (tarea == null) {
